...
export class BaileysAdapter implements IWhatsAppEngine {
  ...
  private async connectInner(): Promise<void> {
    ...
    sock.ev.on('groups.upsert', groups => {
      this.logger.debug('Baileys groups event', { action: 'baileys_groups', event: 'upsert', count: groups?.length ?? 0 });
      const mappedChats = groups.map(g => ({ id: g.id, name: g.subject }));
      this.sessionStore.upsertChats(mappedChats);
    });
    sock.ev.on('groups.update', updates => {
      this.logger.debug('Baileys groups event', { action: 'baileys_groups', event: 'update', count: updates?.length ?? 0 });
      const mappedChats = updates.filter(g => g.subject).map(g => ({ id: g.id, name: g.subject }));
