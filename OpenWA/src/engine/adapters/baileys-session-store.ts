...
export class BaileysSessionStore {
  ...
  upsertContacts(records: Partial<BaileysContactWithPhone>[] = []): void {
    for (const r of records) {
      ...
      const merged: BaileysContactWithPhone = { ...existing };
  ...
  private contactDisplayName(id: string): string | undefined {
    ...
    if (!c) {
      const parsed = parseWaId(id);
      if (parsed.kind === 'user') {
        const normalizedId = `${parsed.userPart}@s.whatsapp.net`;
        c = this.contacts.get(normalizedId);
      }
    }
    return c ? (c.name ?? c.verifiedName ?? c.notify ?? undefined) : undefined;
  }

  private toUnixSeconds(ts: number | string | { toNumber(): number } | null | undefined): number {
    if (ts == null) {
      return 0;
    }
    if (typeof ts === 'number') {
    ...
    return typeof ts.toNumber === 'function' ? ts.toNumber() : 0;
  }
}
