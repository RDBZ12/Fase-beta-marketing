...
export class SessionService implements OnModuleDestroy, OnModuleInit, OnApplicationBootstrap {
  ...
  async getChats(id: string, opts: ListOptions = {}): Promise<any[]> {
    await this.findOne(id); // Verify session exists
    const engine = this.engines.get(id);
    ...
    // 1. Obtener chats activos, grupos totales y contactos sincronizados
    const [baseChats, allGroups, allContacts] = await Promise.all([
      engine.getChats(),
      engine.getGroups(),
      typeof engine.getContacts === 'function' ? engine.getContacts() : Promise.resolve([]),
    ]);

    // 2. Fusionar todo deduplicando por ID
    const chatMap = new Map<string, any>();

    // Metemos todos los grupos primero
    ...
    // Metemos todos los contactos (ignorando grupos que vengan colados aquí)
    for (const c of allContacts) {
      if (!c.id.endsWith('@g.us')) {
        let contactId = c.id;
        
        // Si el contacto viene con un LID, intentar resolver su número de teléfono real
        if (contactId.includes('@lid') && typeof engine.resolveContactPhone === 'function') {
          try {
            const phone = await engine.resolveContactPhone(contactId);
            if (phone) {
              // Asegurarnos de que tenga el formato neutral para cruzar con baseChats
              contactId = phone.includes('@') ? phone : `${phone.replace(/[^0-9]/g, '')}@s.whatsapp.net`;
            }
          } catch (e) {}
        }
    ...
    // Finalmente sobrescribimos con los chats activos (que tienen un timestamp real y lastMessage)
    for (const c of baseChats) {
      // Intentar buscar por el ID exacto, o por el LID resuelto si lo cambiamos
      const existing = chatMap.get(c.id);
      chatMap.set(c.id, {
        ...existing,
        ...c, // priorizar los datos del chat activo (timestamp, unreadCount, lastMessage)
        name: existing?.name || c.name, // preservar nombre si ya lo teníamos de getGroups/getContacts
      });
    }

    // 3. Ordenar: los recientes arriba, los demás (timestamp 0) abajo
    const chats = Array.from(chatMap.values()).sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
    
    // 4. Paginar (límite subido a 10000 para no truncar)
    const paginated = paginate(chats, opts.limit, opts.offset);

    // Enriquecer los chats para devolver la estructura solicitada por el usuario
    const enriched = await Promise.all(
      paginated.map(async chat => {
        let name: string | undefined = chat.name;
        
        // Limpiar el ID (remover @c.us, @g.us, etc)
        const cleanId = chat.id.split('@')[0].split(':')[0];
        
        // Baileys adapter a veces devuelve el ID crudo como fallback en chat.name.
        // Si el nombre es igual al ID limpio, consideramos que NO tiene nombre real.
        const hasNoRealName = !name || name === cleanId || name === chat.id;

        // Si no tiene nombre real, intentar consultarlo dinámicamente
        if (hasNoRealName) {
          if (chat.isGroup) {
            try {
              const info = await engine.getGroupInfo(chat.id);
              if (info?.name) name = info.name;
            } catch (e) {}
          } else {
            try {
              if (typeof engine.getContactById === 'function') {
                const contact = await engine.getContactById(chat.id);
        ...
        return {
          ...chat,
          id: cleanId, 
          originalJid: chat.id, // Mantenemos el original por si acaso
          nombre: name || cleanId, // Fallback al número si no hay nombre definitivo
          tipo: chat.isGroup ? 'grupo' : 'contacto'
        };
      })
    );
