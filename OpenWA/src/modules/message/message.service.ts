...
export class MessageService {
  ...
  async sendImage(sessionId: string, dto: SendMediaMessageDto): Promise<MessageResponseDto> {
    try {
      const engine = this.getEngine(sessionId);
      if (!dto.chatId) throw new BadRequestException('chatId is required');
      ...
      const message = await this.saveOutgoingMessage(sessionId, {
        ...
        type: 'image',
        metadata: {
          media: { mimetype: dto.mimetype, filename: dto.filename, data: dto.base64 ? '[BASE64_DATA]' : dto.url },
        },
      });
      ...
      this.logger.error(`Error in sendImage for session ${sessionId}:`, error instanceof Error ? error.message : String(error));
  ...
  async sendVideo(sessionId: string, dto: SendMediaMessageDto): Promise<MessageResponseDto> {
    try {
      ...
      this.logger.error(`Error in sendVideo for session ${sessionId}:`, error instanceof Error ? error.message : String(error));
      throw this.toClientFacingError(error);
    }
  }
  ...
  async sendAudio(sessionId: string, dto: SendMediaMessageDto): Promise<MessageResponseDto> {
    try {
      ...
      this.logger.error(`Error in sendAudio for session ${sessionId}:`, error instanceof Error ? error.message : String(error));
      throw this.toClientFacingError(error);
    }
  }
  ...
  async sendDocument(sessionId: string, dto: SendMediaMessageDto): Promise<MessageResponseDto> {
    try {
      ...
      this.logger.error(`Error in sendDocument for session ${sessionId}:`, error instanceof Error ? error.message : String(error));
      throw this.toClientFacingError(error);
    }
  }
