...
function AppLayout({ campaigns, fetchCampaigns, loading, searchTerm, setSearchTerm }: any) {
  ...
  useEffect(() => {
    if (!loadingProfile && profile && !activeTab) {
      // Si es cliente (rol 5 o sin rol de staff), va a portal-cliente, si no a dashboard
      if (profile.id_rol >= 5) {
  ...
  }, [profile, loadingProfile, activeTab]);

  const totalLeads     = campaigns.reduce((sum: any, c: any) => sum + c.leads, 0);
  const avgCtr         = campaigns.length > 0
    ? (campaigns.reduce((sum: any, c: any) => sum + c.ctr, 0) / campaigns.length).toFixed(1) : '0';
  const activeCampaigns = campaigns.filter((c: any) => c.status === 'Activa').length;
  ...
  const metrics: Metric[] = [
    ...
    { label: 'ROI Promedio',        value: '312%',                                             change: '+141pp', isPositive: true,  subtext: 'todas las campañas' },
  ];
  ...
  if (loadingProfile || !activeTab) return (
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className="flex-1 p-6 lg:p-8 space-y-6 overflow-y-auto">
        ...
        {activeTab === 'dashboard' && (
          <>
            ...
            {loading ? (
              ...
              <CampaignsTable
                campaigns={filteredCampaigns} searchTerm={searchTerm} setSearchTerm={setSearchTerm}
        ...
        {activeTab === 'portal-cliente' && <ClientGallery campaigns={campaigns} onBackToDashboard={() => setActiveTab('dashboard')} />}
...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      console.log('App.tsx getSession:', session ? 'User found' : 'No user');
      setSession(session);
    });
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, s) => {
      console.log('App.tsx onAuthStateChange:', event, s ? 'User found' : 'No user');
      setSession(s);
    });
