...
export function AdminCampanasModule({ campaigns, fetchCampaigns }: AdminCampanasModuleProps) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    {c.estado_moderacion !== 'aprobada' && c.estado_moderacion !== 'rechazada' && (
                      <>
                        <button 
                          ...
                          className="px-2 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg text-xs flex items-center gap-1 transition-colors border border-blue-200/50"
                        ...
                        <button 
                          ...
                          className="px-2 py-1.5 bg-violet-600 hover:bg-violet-700 text-white font-medium rounded-lg text-xs flex items-center gap-1"
                          title="Ver y Aprobar"
                        >
      ...
      {editingCampaign && (
        <div className="fixed inset-0 z-[1008] flex items-center justify-center bg-black/70 p-4 overflow-y-auto">
           <div className="w-full max-w-5xl bg-white rounded-[10px] shadow-[0_0_40px_rgba(0,0,0,0.4)] relative my-8 animate-blur-in">
             <button onClick={() => setEditingCampaign(null)} className="absolute top-3 right-3 z-50 p-2 bg-slate-100/80 backdrop-blur-sm rounded-full text-slate-500 hover:bg-red-500 hover:text-white transition-colors shadow-sm">
               <X className="w-5 h-5" />
             </button>
